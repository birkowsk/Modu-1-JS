...
function filterWith(arr, phrase) {
    ...
    return arr.filter(function (x) {
        ...
        if (typeof x === "string") {
            const result1 = x.includes(phrase)
            return result1
        }

        if (typeof x === "number") {
            const result2 = x.toString().includes(phrase);
            return result2
        }
        if (Array.isArray(x)) {
            const result3 = filterWith(x, phrase).length > 0;
            return result3
        }

        if (Object.prototype.toString.call(x) === '[object Object]') {
            const arrayValues = Object.values(x)
            const result4 = filterWith(arrayValues, phrase).length > 0
            return result4
        }
...
console.log(filterWith(data, 'Barlow'))
  // console.log(filterWith(data,'tag'));
  // console.log(filterWith(data,'id'));
